...
TAG = os.environ.get("TAG", os.path.basename(CKPT)[:-3]); TTA = {"0": 0, "1": 8}.get(os.environ.get("TE_TTA", "0"), None) or int(os.environ.get("TE_TTA", "0"))   # test-time augmentation folds: 0 off, 4 flips, 8 flips+rotations ("1" = 8, legacy)
...
for n, f in enumerate(files):
    ...
    with torch.no_grad():
        for b0 in range(0, len(coords), BW):
            ...
            xin = torch.cat([dn*kt, kt, gn] + ([at] if AUX else []), 1)
            with torch.autocast("cuda", dtype=torch.bfloat16):
                if TTA:
                    mu, lv = tta_forward(net, xin, ridx, TTA)
                else:
                    mu, lv = net(xin, ridx)
            mu = mu.float()
