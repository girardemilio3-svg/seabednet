...
print(f"corridor: surveyed {sv:,.0f} km2  inferred(<=6km) {fl:,.0f} km2")
json.dump({"surveyed_km2": sv, "inferred_km2": fl}, open("corridor_stats.json", "w"))
...
def base_fig():
    ...
    ax.set_position([0.0, 0.09, 1.0, 0.86])
    return fig, ax
...
ax.imshow(rgbf, extent=ext, origin="upper", aspect=2.3, zorder=2)
ax.imshow(rgbs, extent=ext, origin="upper", aspect=2.3, zorder=3)
annotate(ax)
ax.set_title("THE CHURCHILL CORRIDOR, COMPLETED — blue: surveyed (CHS archive) · amber: SeabedNet v5 inference",
             color="white", fontsize=16, weight="bold", loc="left", pad=12)
fig.text(0.02, 0.055, f"{sv:,.0f} km² surveyed → +{fl:,.0f} km² completed within 6 km of survey lines · "
         f"v5: 48% better than gravity physics near soundings, matches it beyond · per-pixel σ",
...
fig.savefig("corridor_hero.png", dpi=130, facecolor="#04060c", bbox_inches="tight")
print("HERO_DONE")
...
paint_context(ax, ext)
ax.imshow(rgbk, extent=ext, origin="upper", aspect=2.3, zorder=2)
ax.imshow(rgbu, extent=ext, origin="upper", aspect=2.3, zorder=3)
annotate(ax)
ax.set_title("WHERE THE CORRIDOR IS STILL UNCERTAIN — per-pixel σ of the inferred seabed (dark grey: surveyed)",
             color="white", fontsize=16, weight="bold", loc="left", pad=12)
...
fig.text(0.02, 0.02, "SeabedNet v5-small σ (uncalibrated, rank-valid: corr 0.63 with true error) · planning prior — NOT for navigation",
         color="#66779a", fontsize=8.5)
fig.savefig("corridor_sigma.png", dpi=130, facecolor="#04060c", bbox_inches="tight")
print("SIGMA_DONE")
